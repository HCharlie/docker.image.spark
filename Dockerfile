...
FROM quay.io/takaomag/openblas:release-0.3.7-2020.01.08.04.58

ENV \
    X_DOCKER_REPO_NAME=spark \
    X_SPARK_VERSION=3.0.0-preview2 \
    ...
    SPARK_EXECUTOR_URI=file:///opt/local/spark/dist/spark-3.0.0-preview2-bin-3.2.1.tgz \
...
#     [[ "$(cut -d. -f1 <<< ${X_HADOOP_VERSION})" != '3' ]] || sed --in-place -e 's|<id>hadoop-2\.7</id>|<id>hadoop-3\.0</id>|g' -e 's|<hadoop\.version>2\.7\.3</hadoop\.version>|<hadoop\.version>3\.0\.0</hadoop\.version>|g' pom.xml && \
    ...
    export MAVEN_OPTS="-Xmx1g -XX:ReservedCodeCacheSize=512m" && \
